...
export class ReferenceToOneFromForeignKey<ForeignEntity extends AnyEntity, ThisEntity extends AnyEntity> {
  constructor(
  ...
  async getEntity(): Promise<ForeignEntity> {
    ...
    const results = await collection.get((ref) =>
      query(ref, where(this.foreignKey as string, '==', this.thisEntity.getId()), limit(1))
    )
    return results[0]
  }
}
