...
export class ReferenceToMultipleFromForeignKey<ForeignEntity extends AnyEntity, ThisEntity extends AnyEntity> {
  constructor(
  ...
  async getEntities(): Promise<ForeignEntity[]> {
    ...
    return collection.get((ref) => query(ref, where(this.foreignKey as string, '==', this.thisEntity.getId())))
  }
}
