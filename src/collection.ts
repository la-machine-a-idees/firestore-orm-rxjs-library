import { firstValueFrom, map, Observable, shareReplay, Subject, finalize } from 'rxjs'
...
export class Collection<ThisEntity extends AnyEntity> {
  ...
  observe$(queryFn?: (collectionRef: CollectionReference) => Query): Observable<ThisEntity[]> {
    // Get custom query or, by default, create a query to get all documents.
    const aQuery = queryFn ? queryFn(this.firestoreCollectionReference) : query(this.firestoreCollectionReference)
  ...
  get(queryFn?: (collectionRef: CollectionReference) => Query): Promise<ThisEntity[]> {
