...
describe('Query operations', () => {
  ...
  it('should get a family created via Firestore API', async () => {
    ...
    const docRef = await addDoc(familyCollection.firestoreCollectionReference, familyData)
  ...
  it('should query families by city', async () => {
    ...
    await Promise.all(familiesData.map((data) => addDoc(familyCollection.firestoreCollectionReference, data)))

    // Query families by city using get
    const families = await familyCollection.get((ref) => query(ref, where('city', '==', cityName)))
  ...
  it('should observe families being added to a city', async () => {
    ...
    const subscription = familyCollection
      .observe$((ref) => query(ref, where('city', '==', cityName), orderBy('address')))
      .subscribe((families) => {
